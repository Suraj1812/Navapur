...
export interface World { group: THREE.Group; places: Place[]; colliders: Collider[]; districts: District[]; roadCoordinates: number[]; lamps: THREE.Mesh[]; roadMaterial: THREE.MeshStandardMaterial; bounds: number; update(time: number, weather: Weather): void; }
